...
class TriviaClient(discord.Client):
    ...
    def display_points(self, guild_name):
        ...
        for member, points in sorted(self.points.items(), key=lambda x: x[1], reverse=True):
    ...
    def clean_string(self, question):
        question = question.replace('&#039;','\'').replace('&quot;', '\'').replace('&eacute;','e').replace('&ouml;','o')
        return question
    ...
    def get_question(self, category_choice):
        ...
        for question in self.trivia_json['results']:
            if random_mode and count == randint:
                return question
            if category_choice.lower() == self.clean_category(question['category'].lower()):
                return question
            if category_choice.lower() == self.clean_category(question['category'].lower())[:3]:
                return question
    ...
    async def on_message(self, message):
        ...
        if message.content.startswith('!answer') or message.content.startswith('!ans') or message.content.startswith('!a'):
            ...
            if self.trivia_json and self.category_choice:
                ...
                if self.check_answer(answer):
                    ...
                    correct = False
                    await message.channel.send('Sorry, that\'s not right. The correct answer is {}'.format(self.correct_answer))
        ...
        if message.content.startswith('!category') or message.content.startswith('!cat') or message.content.startswith('!c'):
            if self.trivia_json:
                if not self.category_choice:
                    ...
                    if question:
                        ...
                        self.correct_answer = self.clean_string(question['correct_answer'])
                        self.incorrect_answers = [self.clean_string(question) for question in question['incorrect_answers']]

                        self.answer_set = set([self.correct_answer] + self.incorrect_answers)
                        question_str = dedent('''
Question: {}
...
NOTE: Please format your response like: '!answer answer_choice' or '!ans answer_choice' or '!a answer_choice'
                        ...
                        await message.channel.send('Please choose a valid category from the list!')

                
                else:
                    await message.channel.send('''You have already chosen a category, answer the question before requesting another one!''')
        ...
        if message.content.startswith('!trivia') or message.content.startswith('!tri') or message.content.startswith('!t'):
            author = message.author.name
            if not self.trivia_json:
                if len(message.content.split(' ')) > 1:
                    self.num_questions = message.content[message.content.index(' ') + 1:]
                else:
                ...
                choose_cat_str = dedent('''
                    ...
                    '''.format(author, ', '.join(categories)))

                await message.channel.send(choose_cat_str)
            else:
                if not self.category_choice:
                    await message.channel.send('{} has already requested a trivia question! Please choose a category.'.format(author))
                else:
                       await message.channel.send('A question has already been asked! Please answer before requesting another question.')
